...
volatile int *KEY_ptr = (int *)KEY_BASE; // push buttons
volatile int *PS2_ptr = (int *)PS2_BASE; // ps2 keyboard
...
int main(void)
{
	int easy_high_score = 0;
	int medium_high_score = 0;
	int hard_high_score = 0;
	int v_hard_high_score = 0;
	while (true)
	{
		...
		*(PS2_ptr) = 0xFF; // reset
		...
		while (choice == false)
		{
			...
			{ // press KEY[0] = easy
				game.level = easy;
				b = (char)easy_high_score % 10;
				a = (char)easy_high_score / 10;
				HEX_PS2_1(a, b);
				NUM_ROWS = 1;
				NUM_COLS = 4;
				choice = true;
			...
			{ // press KEY[1] = medium
				game.level = medium;
				b = (char)medium_high_score % 10;
				a = (char)medium_high_score / 10;
				NUM_ROWS = 2;
				NUM_COLS = 4;
				choice = true;
			}
			...
			{ // press KEY[2] = hard
				game.level = hard;
				b = (char)hard_high_score % 10;
				a = (char)hard_high_score / 10;
				NUM_ROWS = 3;
				NUM_COLS = 4;
				choice = true;
			}
			else if (*KEY_ptr == 0b1000)
			{ // press KEY[3] = very hard
				game.level = v_hard;
				b = (char)v_hard_high_score % 10;
				a = (char)v_hard_high_score / 10;
				NUM_ROWS = 4;
				NUM_COLS = 4;
				choice = true;
			}
			...
			if (flippedCard != -1)
			{
				...
				if (game.pressedCard != -1 && !game.cards[flippedCard].isFlipped)
				{
					...
					if (game.numFinished == NUM_CARDS / 2)
					{
						...
						if (game.level == easy)
						{
							if (game.score > easy_high_score)
							{
								easy_high_score = game.score;
								b = (char)easy_high_score % 10;
								a = (char)easy_high_score / 10;
								HEX_PS2_1(a, b);
							}
						...
						{
							if (game.score > medium_high_score)
							{
								medium_high_score = game.score;
								b = (char)medium_high_score % 10;
								a = (char)medium_high_score / 10;
								HEX_PS2_1(a, b);
							}
						...
						if (game.level == hard)
						{
							if (game.score > hard_high_score)
							{
								hard_high_score = game.score;
								b = (char)hard_high_score % 10;
								a = (char)hard_high_score / 10;
								HEX_PS2_1(a, b);
							}
						}
						if (game.level == v_hard)
						{
							if (game.score > v_hard_high_score)
							{
								v_hard_high_score = game.score;
								b = (char)v_hard_high_score % 10;
								a = (char)v_hard_high_score / 10;
								HEX_PS2_1(a, b);
							}
